...
/* ─────────────────────────────────────────
   14. IMAGE LIGHTBOX
   - Portfolio images pe click karo → full screen open hoga
   - Keyboard: Esc = close, ← → = navigate
   - Touch: swipe left/right = navigate
───────────────────────────────────────── */
(function () {
  // Saari portfolio images collect karo (order maintain karne ke liye)
  var lbImages  = [];
  var lbCurrent = 0;
  ...
  function buildImageList() {
    ...
    document.querySelectorAll('.port-card').forEach(function (card) {
      var src = card.getAttribute('onclick');
      if (!src) return;
      // onclick="openLightbox('assets/images/port1.jpg')" se src nikalo
      var match = src.match(/openLightbox\(['"]([^'"]+)['"]\)/);
      if (match) lbImages.push(match[1]);
    });
  }
  ...
  function showImage(index) {
    ...
    // Prev/Next buttons — hide agar sirf ek image ho
  ...
  window.openLightbox = function (src) {
    ...
    // Is src ka index dhundo
    var idx = lbImages.indexOf(src);
    if (idx === -1) {
      // Fallback: list mein add karo
      lbImages.push(src);
      idx = lbImages.length - 1;
    }
